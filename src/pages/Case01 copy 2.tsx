...
const Section = ({ title, children }: SectionProps) => (
  <div className="w-full">
    <div className="max-w-screen-lg mx-auto pb-12 flex flex-nowrap space-x-3">
      ...
      <div className="basis-5/7">{children}</div>
    </div>
  </div>
);
...
export default function Case01() {
  return (
    <div className="bg-zinc-950 w-full text-gray-200">
      <div className="max-w-screen-lg mx-auto flex flex-nowrap space-x-1 pt-20 pb-12">
        <div className="basis-5/7">
          <h1 className="text-indigo-400 pb-4">
            Product case 01: Scalable, Multi-Panel Workspace for High-Volume Sales Workflows
          </h1>
          <h2 className="text-white">VanillaSoft Caller UI Redesign</h2>
          <h3 className="pt-4">
      ...
      {/* Image */}
      <div className="w-full">
        <div className="max-w-screen-lg mx-auto pb-12 flex flex-nowrap">
          <img src={image001} alt="VanillaSoft UI overview" />
        </div>
      </div>
...
{/* Caller UI User Journey - Responsive */}
<div className="w-full bg-zinc-900 text-gray-200 py-12 mb-12">
  <div className="max-w-screen-xl mx-auto px-4">
    <h3 className="text-indigo-400 text-2xl pb-8">User Journey: Caller UI Workflow</h3>

    {/* Desktop Table */}
    <div className="hidden md:grid grid-cols-1 md:grid-cols-6 gap-4 border-b border-gray-700 pb-4 font-semibold text-white">
      <div>Stage</div>
      <div>User Goals</div>
      <div>Actions</div>
      <div>Pain Points</div>
    ...
    <div className="hidden md:block">
      {journeyData.map((row, i) => (
        <div
          key={i}
          className={`grid grid-cols-1 md:grid-cols-6 gap-4 py-4 border-b border-gray-800 ${i % 2 === 0 ? 'bg-zinc-900' : 'bg-zinc-800'}`}
        >
          <div className="px-4 py-2">{row.stage}</div>
          <div className="px-4 py-2">{row.goals}</div>
          <div className="px-4 py-2">{row.actions}</div>
          <div className="px-4 py-2">{row.painPoints}</div>
          <div className="col-span-2 px-4 py-2">{row.opportunities}</div>
        </div>
      ))}
    </div>
    ...
    <div className="md:hidden space-y-4">
      {journeyData.map((row, i) => (
        ...
        >
          ...
          <div className="mt-2 space-y-2 text-gray-200">
            <p><strong>User Goals:</strong> {row.goals}</p>
            <p><strong>Actions:</strong> {row.actions}</p>
            <p><strong>Pain Points:</strong> {row.painPoints}</p>
            <p><strong>Opportunities / Solutions:</strong> {row.opportunities}</p>
          </div>
        </details>
      ))}
    </div>
      ...
      <Section title="Hypothesis">
        ...
        <ul className="list-disc list-inside space-y-2 pb-4">
          <li><strong>Work faster</strong> with fewer interruptions</li>
          <li><strong>Make fewer errors</strong> thanks to streamlined workflows</li>
          <li><strong>Experience less cognitive load</strong>, staying focused on high-value tasks</li>
        </ul>
        <p>
          This approach transforms the Caller UI from a fragmented set of windows into a <strong>centralized, efficient workspace</strong> that supports users’ natural workflow and enhances overall productivity.
        </p>
      </Section>
      ...
      <Section title="Results / Impact">
        <ul className="list-disc list-inside space-y-1">
          <li>Productivity increase: <strong>30%–100%</strong> reported by users</li>
          <li>Cognitive load reduction: Consolidated multiple tools into one interface</li>
          <li>User satisfaction: <strong>4.7-star</strong> rating on Capterra, <strong>4.6</strong> on G2</li>
      ...
      {/* Caller UI Image */}
      <div className="w-full">
        <div className="max-w-screen-xl mx-auto pb-12 flex flex-nowrap">
          <img src={image002} alt="VanillaSoft Caller UI overview" />
        </div>
      </div>
    </div>
  );
